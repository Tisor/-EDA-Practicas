...
public class LinkedBinaryTree<E> implements BinaryTree<E> {
    ...
    public Position<E> removeSubtree(Position<E> p) 
    	...
    	BTNode<E> node = checkPosition(p);
    	BTNode<E> parent = node.getParent();
    	// Comprobamos si el nodo es raíz
    	if(parent != null){
        	//Comprobamos si es el hijo izquierdo
        	// o el hijo derecho
        	if(node == parent.getLeft()){
        		parent.setLeft(null);
        	}
        	...
        	node.setParent(null);
        	int removedE = calculateSize(node);
        	size = size - removedE;
    	}
    	// Si es raiz lo apuntamos a null, y el tamaño pasa a ser 0
    	else{
    		node = null;
    		size = 0;
    		this.root = null;
