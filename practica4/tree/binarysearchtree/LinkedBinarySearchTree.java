...
import practica2.tree.binarytree.LinkedBinaryTree;
import practica4.iterator.InorderIterator;
...
public class LinkedBinarySearchTree<E> implements BinarySearchTree<E> {
	...
	public Iterable<Position<E>> findRange(E minimum, E maximum) {
		ArrayList<Position<E>> result = new ArrayList<>(); 
		//Cogemos la ra�z
		Position<E> r = binTree.root();
		if(binTree.hasLeft(r)){
			findRangeAux(binTree.left(r), minimum, maximum, result);
		}
		if(comparator.compare(minimum, r.getElement()) >= 0 &&
				(comparator.compare(r.getElement(), maximum) <= 0)){
			result.add(r);
		}
		if(binTree.hasRight(r)){
			findRangeAux(binTree.right(r), minimum, maximum, result);
		}
		return result;
	}
	
	public void findRangeAux(Position<E> subtree, E minimum, E maximum, ArrayList<Position<E>> result){
		if(comparator.compare(minimum, subtree.getElement()) >= 0 &&
				(comparator.compare(subtree.getElement(), maximum) <= 0)){
			result.add(subtree);
		}
		if(binTree.hasLeft(subtree)){
			findRangeAux(binTree.left(subtree), minimum, maximum, result);
		}
		if(binTree.hasRight(subtree)){
			findRangeAux(binTree.right(subtree), minimum, maximum, result);
		}
	}
