...
//        }
	...
	public Iterable<Position<E>> findRange(E minimum, E maximum) {
		Iterator<Position<E>> iter = this.iterator();
		ArrayList<Position<E>> res = new ArrayList<>();
		while(iter.hasNext()){
			Position<E> aux = iter.next();
			if(comparator.compare(minimum, aux.getElement()) >= 0 &&
					comparator.compare(aux.getElement(), maximum) <= 0){
				res.add(aux);
			}
		}
		return res;
