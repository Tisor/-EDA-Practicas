...
public class LinkedTree<E> implements Tree<E> {
    ...
    */
    public Position<E> moveSubtree(Position<E> pOrig, Position<E> pDest) throws IllegalStateException{
		...
		if(isRoot(pOrig)){
			throw new IllegalStateException("Can't move the root");
		}
		else{
			//Remove the element from the parent
			TreeNode<E> nodeAux = nodeOrig.getParent(); 
			nodeAux.getChildren().remove(nodeOrig);
			
			//nodeOrig is added to the list of pDest children's
    ...
    public Position<E> moveSubtree(Position<E> pOrig, Position<E> pDest, LinkedTree<E> newTree) throws IllegalStateException{
		TreeNode<E> nodeOrig = checkPosition(pOrig);
		TreeNode<E> nodeDest = newTree.checkPosition(pDest);
		if(isRoot(pOrig)){
			throw new IllegalStateException("Can't move the root");
		}
		else{
			Iterator<Position<E>> iter = this.iteratorFactory.createIterator(this, pOrig);
			int cont = 0;
			while(iter.hasNext()){
				TreeNode<E> n = checkPosition(iter.next());
				n.myTree = newTree;
				cont++;
			}
			this.size -= cont; 
			...
			newTree.setSize(newTree.size() + cont);
		}
		return nodeDest;
